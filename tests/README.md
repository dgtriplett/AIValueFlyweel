...
| `test_user_management.py` | Admin user-management CRUD (admin-portal Phase B): `GET/PUT/DELETE /api/users` are **all admin-gated and fail closed** — a non-admin, an empty `GRID_ATLAS_ADMINS`, or no trusted identity gets a 403 before any row is read or written. PUT upserts the role AND writes an `audit_log` row (`entity_type='app_user'`, `role_grant`), normalizes the email, and rejects a role outside {admin, pm, executive} with 422. GET reports `is_env_admin` for allowlisted emails (including allowlist admins with no row), because the allowlist outranks the table. |
