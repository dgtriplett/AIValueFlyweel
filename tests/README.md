...
| `test_crash_guards.py` | Three fresh-install / bad-input **500s** from the production-readiness audit. The onboarding template built its dropdown range as `f"E2:E{max_row}"`, which on an empty database spells the reversed `E2:E1` — so the FIRST workbook a new customer downloads 500'd; asserts a parseable workbook, headers intact, and that the dropdown returns once rows exist (an over-correction that deleted it would also pass the empty case). `/api/impact/{node_id}` did `int(nid.split("-")[1])`, turning any malformed id into IndexError/ValueError instead of the **422** the typed `{id:int}` siblings already return. And several `fetchrow(... RETURNING ...)` writes indexed a `None` result under a degraded pool, raising `TypeError` where the rest of the app raises the actionable 503 — pinned per-site, plus an AST sweep over the CRUD route modules so a NEW unguarded write fails here rather than in the next audit. |
